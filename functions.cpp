...
std::string HashFunkcija(std::string tekstas){
      ...
      constexpr int HASH_DYDIS = 32; // 256 bitai – saugesnis dydis
    unsigned char hash[HASH_DYDIS];

    // Saugesnis pradinis seed užpildymas (salting)
    for(int i = 0; i < HASH_DYDIS; i++)
        hash[i] = i * 89 + 157;

    // Efektyvesnis apdorojimo ciklas
    for(size_t i = 0; i < tekstas.size(); ++i){
        unsigned char t = tekstas[i];
        int pos = (i * t + t) % HASH_DYDIS;
        hash[pos] ^= t;
        hash[pos] = ((hash[pos] << (t % 5 + 1)) | (hash[pos] >> (8 - (t % 5 + 1))));
        hash[(pos + 17) % HASH_DYDIS] ^= t >> 2;
        hash[(pos + 7) % HASH_DYDIS] += (t ^ (i * 13));
    }

    // Saugesnis finalinis miksavimas
    for(int round = 0; round < 64; ++round) {
        for(int i = 0; i < HASH_DYDIS; ++i){
            hash[i] = (hash[i] + hash[(i+3)%HASH_DYDIS]) ^ (i*31 + round*17);
            hash[i] = ((hash[i] << ((hash[i]+round)%7+1)) | (hash[i] >> (8 - ((hash[i]+round)%7+1))));
            hash[i] = (hash[i] * 31 + ((round * 17 + i) & 0xFF));
        }
    }

    // Užtikrintas pilnas hex užpildymas
    static const char hexmap[] = "0123456789abcdef";
    std::string out;
    out.reserve(HASH_DYDIS * 2);
    for(int i = 0; i < HASH_DYDIS; i++){
        out.push_back(hexmap[hash[i] >> 4]);
        out.push_back(hexmap[hash[i] & 0x0F]);
    }
    return out;
}
